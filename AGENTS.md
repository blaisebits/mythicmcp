...
Each reference has its own AGENTS.md with detailed architecture notes. Consult these when implementing Mythic integrations.
...
Use normal git branches and focused commits. Release preparation is handled by the scripts in `scripts/` and the GitHub workflows in `.github/workflows/`.

## Key Directories

- `src/mythicmcp/` - MCP server, core tools, config, models, and Mythic connection helpers
- `src/mythicmcp/plugins/builtin/` - Bundled YAML plugins for Apollo, Poseidon, and Arachne
- `tests/unit/` - Unit tests that should run without a live Mythic server
- `tests/integration/` - Optional live Mythic integration tests driven by local config
- `scripts/` - Release checks, helper scripts, and Docker Codex harness utilities
...
- Python 3.10+ + mcp>=1.26.0, mythic>=0.2.10, pydantic>=2.0.0 (all existing) (010-core-payload-tools)
- N/A (stateless — queries Mythic server) (010-core-payload-tools)

## Installation

Install as a global tool using uv directly from GitHub. This project is not currently published to PyPI.
...
Before creating or pushing a release tag, run the release preflight script for your platform:
...
You can pass an explicit tag, for example `v0.1.0`. If omitted, the scripts use `v<project.version>` from `pyproject.toml`.

The release scripts validate:
- tag matches `pyproject.toml` version
- working tree is clean
- current commit is reachable from `origin/master`
- local and remote tag do not already exist
- dependencies sync with `uv sync --locked --python 3.11`
- unit tests pass with `pytest tests/unit -q`
- package builds with `uv build`

If the scripts pass, create and push an annotated tag:
...
Prefer tagging a commit that already passed CI on `master`. Avoid moving public tags. If a tag was pushed and release verification failed before publication, moving it is technically possible but should be treated as private/pre-release cleanup only.

## GitHub Workflows

The repo uses two GitHub Actions workflows:

- `ci.yml` runs only on pushes to `master` and manual dispatch. It runs unit tests and `uv build`.
- `release.yml` runs only on pushed tags matching `v*`. It verifies the tagged commit is on `origin/master`, runs tests/build with read-only permissions, then publishes a GitHub release in a separate job with write permissions.

Security choices to preserve:
- do not add `pull_request` triggers without a dedicated security review
- keep release verification and publishing split into separate jobs
- keep `uv sync --locked` in CI/release jobs
- keep Git hooks disabled in workflows with `core.hooksPath /dev/null`
- keep checkout credentials from persisting unless a workflow truly needs them

The release workflow does not publish to PyPI. It creates a GitHub release and uploads the built `dist/*` artifacts.

## Testing Notes

For a clean local unit test run, clear plugin-loading environment variables that can change the registered MCP tool surface:
...
Do not delete or rebuild `.venv` while a live Codex MCP server is running from this repo. Stop/disable the MCP server first, or use a separate test environment.
...
## Subagent MCP Testing

Codex subagents can be used to test the live MCP surface directly. In practice they inherit the current Mythic MCP tool/resource metadata, so this is a good way to validate wording changes without restarting the main thread.

Recommended method:
- spawn a subagent with the current thread context
- ask it to inspect specific Mythic MCP tools/resources and report the visible descriptions
- for workflow tests, constrain it to a small allowlist of tools and require it to infer behavior from live MCP metadata plus task results only
- use the subagent report to validate what a fresh agent would actually see

Good fit:
- checking tool description wording
- checking resource visibility/readability
- checking whether execution hints are strong enough for a fresh agent
...
### Payload Tools
- `core_list_payloads` - List all payloads in current operation
- `core_get_payload` - Get detailed payload info by UUID
- `core_create_payload` - Create and build a new standard payload
- `core_download_payload` - Download built payload binary by UUID
- `core_check_payload_config` - Validate payload C2 configuration
- `core_payload_redirect_rules` - Get redirect rules for a payload

## Recent Changes
- 010-core-payload-tools: Added Python 3.10+ + mcp>=1.26.0, mythic>=0.2.10, pydantic>=2.0.0 (all existing)
