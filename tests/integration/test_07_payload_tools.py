...
class TestCreatePayload:
    ...
    async def test_create_payload_with_named_c2_instance(
        self, mythic_instance, integration_config: IntegrationTestConfig
    ):
        """Create a payload by resolving a saved C2 instance name in the current op."""
        from mythicmcp.tools.c2profiles import create_c2_instance
        from mythicmcp.tools.payloads import create_payload

        agent = integration_config.agents[0]
        primary_profile = agent.c2_profiles[0]
        instance_name = f"mcp-payload-{uuid4().hex[:8]}"

        await create_c2_instance(
            mythic_instance,
            instance_name=instance_name,
            c2_profile_name=primary_profile.c2_profile,
            c2_parameters=primary_profile.c2_profile_parameters,
        )
        ...
        result = await create_payload(
            mythic_instance,
            payload_type_name=agent.payload_type,
            filename=f"mcp_payload_named_instance_{agent.name}",
            operating_system=agent.os,
            c2_instances=[{"instance_name": instance_name}],
            build_parameters=build_parameters or None,
            description="MythicMCP payload named C2 instance integration test",
            timeout=integration_config.timeouts.payload_generation,
        )
