...
class TestPayloadC2InstanceInputs:
    """Test named C2 instance helpers used by core_create_payload."""

    def test_parse_c2_instances_valid(self):
        from mythicmcp.tools.payloads import _parse_c2_instances_json

        result = _parse_c2_instances_json('[{"instance_name": "default"}]')
        assert result == [{"instance_name": "default"}]

    def test_parse_c2_instances_missing_name(self):
        from mythicmcp.tools.payloads import InvalidJSONError, _parse_c2_instances_json

        with pytest.raises(InvalidJSONError):
            _parse_c2_instances_json('[{"wrong_key": "default"}]')

    def test_parse_c2_instances_non_string_name(self):
        from mythicmcp.tools.payloads import InvalidJSONError, _parse_c2_instances_json

        with pytest.raises(InvalidJSONError):
            _parse_c2_instances_json('[{"instance_name": 1}]')

    def test_parse_c2_instances_rejects_extra_keys(self):
        from mythicmcp.tools.payloads import InvalidJSONError, _parse_c2_instances_json

        with pytest.raises(InvalidJSONError):
            _parse_c2_instances_json(
                '[{"instance_name": "default", "c2_profile_parameters": {"callback_host": "https://test"}}]'
            )

    @pytest.mark.asyncio
    async def test_create_payload_resolves_named_c2_instances(self, monkeypatch: pytest.MonkeyPatch):
        ...
        async def fake_resolve(mythic_instance, instance_name: str):
            class ResolvedInstance:
                c2_profile_name = "http"
                c2_parameters = {"callback_host": "https://test"}

            assert instance_name == "default"
            return ResolvedInstance()

        async def fake_create_payload(**kwargs):
            captured.update(kwargs)
            return {"uuid": "payload-uuid", "build_phase": "success", "build_message": "ok"}

        async def fake_execute_custom_query(**kwargs):
            return {
                "payloadtype": [
                    {"name": "apollo", "container_running": True},
                ]
            }

        monkeypatch.setattr(
            "mythicmcp.tools.payloads.resolve_c2_instance_by_name",
            fake_resolve,
        )
        monkeypatch.setattr(mythic_module, "execute_custom_query", fake_execute_custom_query)
        monkeypatch.setattr(mythic_module, "create_payload", fake_create_payload)

        mythic_instance = type("MythicStub", (), {"current_operation_id": 1})()
        ...
        result = await create_payload(
            ...
            c2_instances=[{"instance_name": "default"}],
        )

        assert result.uuid == "payload-uuid"
        assert captured["c2_profiles"] == [{
            "c2_profile": "http",
            "c2_profile_parameters": {"callback_host": "https://test"},
        }]

    @pytest.mark.asyncio
    async def test_create_payload_rejects_mixed_c2_inputs(self):
        from mythicmcp.tools.payloads import InvalidPayloadInputError, create_payload

        mythic_instance = type("MythicStub", (), {"current_operation_id": 1})()

        with pytest.raises(InvalidPayloadInputError):
    ...
    @pytest.mark.asyncio
    async def test_create_payload_rejects_missing_payload_type(self, monkeypatch: pytest.MonkeyPatch):
        from mythicmcp.tools.payloads import PayloadTypeUnavailableError, create_payload

        async def fake_execute_custom_query(**kwargs):
            return {
                "payloadtype": [
                    {"name": "poseidon", "container_running": True},
                ]
            }
        ...
        with pytest.raises(PayloadTypeUnavailableError, match="Available payload types: poseidon"):
            await create_payload(
                mythic_instance,
                payload_type_name="apollo",
                filename="test.exe",
    ...
    @pytest.mark.asyncio
    async def test_create_payload_rejects_stopped_payload_container(self, monkeypatch: pytest.MonkeyPatch):
        from mythicmcp.tools.payloads import PayloadTypeUnavailableError, create_payload

        async def fake_execute_custom_query(**kwargs):
            return {
                "payloadtype": [
                    {"name": "apollo", "container_running": False},
                ]
            }
        ...
        with pytest.raises(PayloadTypeUnavailableError, match="container is not running"):
            await create_payload(
                mythic_instance,
                payload_type_name="apollo",
                filename="test.exe",
