...
from mythicmcp.tools.c2profiles import (
    C2InstanceAmbiguousError,
    C2InstanceNotFoundError,
    resolve_c2_instance_by_name,
)
...
class PayloadTypeUnavailableError(PayloadError):
    """Raised when a payload type is missing or its container is not running."""
...
class InvalidPayloadInputError(PayloadError):
    """Raised when payload creation inputs are invalid."""
...
def _parse_c2_instances_json(json_str: str) -> list[dict]:
    """Parse and validate named C2 instance JSON string."""
    if not json_str or not json_str.strip():
        return []
    ...
    try:
        ...
        raise InvalidJSONError("c2_instances", str(e))

    if not isinstance(instances, list):
        raise InvalidJSONError("c2_instances", "Must be a JSON array")

    for i, instance in enumerate(instances):
        if not isinstance(instance, dict):
            raise InvalidJSONError("c2_instances", f"Item {i} must be an object")
        if "instance_name" not in instance:
            raise InvalidJSONError("c2_instances", f"Item {i} missing 'instance_name' key")
        if not isinstance(instance["instance_name"], str) or not instance["instance_name"].strip():
            raise InvalidJSONError("c2_instances", f"Item {i} has invalid 'instance_name'")

        extra_keys = set(instance) - {"instance_name"}
        if extra_keys:
            extras = ", ".join(sorted(extra_keys))
            raise InvalidJSONError(
                "c2_instances",
                f"Item {i} has unsupported keys: {extras}",
            )
...
async def _resolve_c2_instances(
    mythic_instance: mythic_classes.Mythic,
    c2_instances: list[dict],
) -> list[dict]:
    """Resolve saved C2 instance names into Mythic payload c2_profiles entries."""
    resolved_profiles = []
    for instance in c2_instances:
        resolved = await resolve_c2_instance_by_name(
            mythic_instance,
            instance_name=instance["instance_name"],
        )
        resolved_profiles.append(
            {
                "c2_profile": resolved.c2_profile_name,
                "c2_profile_parameters": resolved.c2_parameters,
            }
        )
    return resolved_profiles


async def _validate_payload_type_available(
    mythic_instance: mythic_classes.Mythic,
    payload_type_name: str,
) -> None:
    """Ensure the target payload type exists in Mythic and its container is running."""
    from mythic import mythic

    query = """
    query ListPayloadTypesForBuild {
    ...
    payload_types = result.get("payloadtype", [])
    target = next((p for p in payload_types if p.get("name") == payload_type_name), None)
    available = [p.get("name", "") for p in payload_types if p.get("name")]

    if target is None:
        available_list = ", ".join(available) if available else "none"
        raise PayloadTypeUnavailableError(
            f"Payload type '{payload_type_name}' is not installed or not registered on the Mythic server. "
            f"Available payload types: {available_list}"
        )

    if not target.get("container_running", False):
        raise PayloadTypeUnavailableError(
            f"Payload type '{payload_type_name}' is installed but its Mythic container is not running"
        )
...
async def create_payload(
    ...
    c2_profiles: list[dict] | None = None,
    c2_instances: list[dict] | None = None,
    ...
    if c2_profiles and c2_instances:
        raise InvalidPayloadInputError(
            "c2_profiles and c2_instances are mutually exclusive"
        )

    if c2_instances:
        resolved_c2_profiles = await _resolve_c2_instances(mythic_instance, c2_instances)
    elif c2_profiles:
        resolved_c2_profiles = c2_profiles
    else:
        raise InvalidPayloadInputError(
            "Either c2_profiles or c2_instances is required"
        )

    try:
        await _validate_payload_type_available(mythic_instance, payload_type_name)
        ...
        result = await mythic.create_payload(
            ...
            c2_profiles=resolved_c2_profiles,
            commands=commands,
        ...
        InvalidPayloadInputError,
        C2InstanceNotFoundError,
        C2InstanceAmbiguousError,
        PayloadTypeUnavailableError,
    ):
        raise
    except Exception as e:
        error_msg = str(e).lower()
...
async def core_create_payload(
    ...
    has_c2_profiles = bool(c2_profiles and c2_profiles.strip())
    has_c2_instances = bool(c2_instances and c2_instances.strip())

    if has_c2_profiles and has_c2_instances:
        return CreatePayloadErrorResponse(
            error="c2_profiles and c2_instances are mutually exclusive",
            error_type="invalid_input",
        )
    ...
    try:
        parsed_c2_instances = _parse_c2_instances_json(c2_instances)
    except InvalidJSONError as e:
        return CreatePayloadErrorResponse(error=str(e), error_type="invalid_input")
    ...
    try:
        return await create_payload(
            ...
            filename=filename,
            operating_system=operating_system,
            c2_profiles=parsed_c2 or None,
            c2_instances=parsed_c2_instances or None,
    ...
    except (InvalidPayloadInputError, C2InstanceNotFoundError, C2InstanceAmbiguousError) as e:
        return CreatePayloadErrorResponse(error=str(e), error_type="invalid_input")
    except PayloadTypeUnavailableError as e:
        return CreatePayloadErrorResponse(
            error=str(e),
            error_type="payload_type_unavailable",
        )
    except NoOperationError as e:
