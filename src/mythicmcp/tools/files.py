...
class InvalidUploadInputError(FileError):
    """Raised when upload arguments are missing or conflicting."""
...
def _decode_base64_content(content: str) -> bytes:
    ...
    try:
        ...
        raise InvalidBase64Error(
            "Provide the complete file as one valid base64 string with no chunk "
            f"markers, labels, or other non-base64 text. Decoder error: {e}"
        )


def _read_local_file(file_path: str) -> bytes:
    """Read a local file from disk for upload."""
    try:
        return Path(file_path).read_bytes()
    except Exception as e:
        raise FileUploadError(file_path, f"Unable to read local file: {e}")
...
async def core_upload_file(
    ...
    try:
        ...
        has_file_path = bool(file_path.strip())

        if has_content and has_file_path:
            raise InvalidUploadInputError(
                "Specify either 'content' or 'file_path', not both."
            )
        if not has_content and not has_file_path:
            raise InvalidUploadInputError(
                "Specify either 'content' or 'file_path' for the file to upload."
            )

        if has_file_path:
            content_bytes = _read_local_file(file_path)
            resolved_filename = filename.strip() or Path(file_path).name
        else:
            content_bytes = _decode_base64_content(content)
            resolved_filename = filename.strip()

        # Upload file
        return await upload_file(mythic_ctx.mythic, resolved_filename, content_bytes)
    ...
    except InvalidUploadInputError as e:
        return UploadFileErrorResponse(
            error=str(e),
            error_type="invalid_input",
        )
