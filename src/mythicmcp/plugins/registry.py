...
class PluginRegistry:
    ...
    def mark_agent_registered(self, agent_name: str) -> None:
        """Mark an agent as having its tools registered with MCP.

        Args:
            agent_name: The agent name to mark as registered.
        """
        self._registered_agents.add(agent_name)

    def mark_agent_unregistered(self, agent_name: str) -> None:
        """Mark an agent as no longer having its tools registered with MCP.

        Args:
            agent_name: The agent name to mark as unregistered.
        """
        self._registered_agents.discard(agent_name)

    def is_agent_registered(self, agent_name: str) -> bool:
        """Check if an agent's tools are currently registered with MCP.

        Args:
            agent_name: The agent name to check.

        Returns:
            True if the agent's tools are registered with MCP.
        """
        return agent_name in self._registered_agents

    def list_registered_agents(self) -> list[str]:
        """List agents whose tools are currently registered with MCP.
