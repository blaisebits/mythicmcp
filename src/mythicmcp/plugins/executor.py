...
async def get_callback_agent_type(ctx: Context, callback_id: int) -> CallbackAgentInfo:
    ...
    return CallbackAgentInfo(
        ...
        display_id=callback_data.get("display_id", callback_id),
...
async def execute_task(
    ...
    try:
        async with asyncio.timeout(timeout):
            task_parameters = parameters if parameters else ""
...
async def execute_with_validation(
    ...
    try:
        # Validate agent type
        callback_info = await validate_agent_type(ctx, callback_id, expected_agent_type)

        # Execute task
        result = await execute_task(
            ctx=ctx,
            callback_id=callback_info.display_id,
            command_name=command_name,
