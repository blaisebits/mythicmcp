...
VALID_PARAM_TYPES = frozenset({"string", "integer", "boolean", "beacon_args"})

BeaconArgValue = str | int | bool
BeaconArgPair = tuple[str, BeaconArgValue]
BeaconArgs = str | list[BeaconArgPair]
...
class ParameterConfigModel(BaseModel):
    ...
    def validate_constraints(self) -> ParameterConfigModel:
        ...
        if self.default is not None:
            if self.type == "beacon_args":
                if not isinstance(self.default, (str, list)):
                    raise ValueError(
...
class CommandConfigModel(BaseModel):
    ...
    @field_validator("parameter_mode")
    @classmethod
    def validate_parameter_mode(cls, v: str) -> str:
        if v not in ("dict", "raw_string"):
            raise ValueError("parameter_mode must be 'dict' or 'raw_string'")
        return v
...
def build_parameter_model(
    ...
    field_definitions["callback_id"] = (
        ...
        Field(
            ...,
            description="Canonical callback_id to execute on (not the UI display_id)",
        ),
    )
...
def build_handler(
    ...
    uses_raw_string = command_config.parameter_mode == "raw_string"
    ...
    async def handler(ctx: Context, params: Any) -> Any:
        ...
        # Some Mythic commands parse a raw command-line string instead of JSON args.
        if uses_raw_string:
            if len(task_param_names) != 1:
                raise ValueError(
                    f"Command '{command_config.name}' uses raw_string parameter_mode "
                    "but does not define exactly one task parameter"
                )
            task_params = getattr(params, task_param_names[0])
        else:
            task_params = {}
